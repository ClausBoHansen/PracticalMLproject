...
trainModel <- function(methodtag, methodname, tunegrid = NULL, method = "repeatedcv", folds = 10, repeats = 10){
  ...
  cat(paste("Training model ", methodtag," with method ", method, "\n", sep = ""))
...
methodtag <- "LMT"
methodname <- "Logistic Model Trees"
reps <- 5

TrainedModel <- trainModel(methodtag, methodname, repeats = reps)
TrainedModel
plot(TrainedModel, main = paste(methodname, reps, "repeats"))
...
methodtag <- "glm"
methodname <- "Logistric Regression"
reps <- 10

TrainedModel <- trainModel(methodtag, methodname, logging = TRUE, repeats = reps)
TrainedModel

finalModel.glm = TrainedModel
...
#selecting subset of variables, since using all variables is creating errors
selectedPreds = c(
  'PRO_msrp',
  'CAS_Initial_Return_Code_C2',
  'CAS_Initial_Return_Code_C5',
  'CAS_WarrantyStatus_N',
  'PRO_Segment_PND',
  'PRO_Family_FIT_VIVOSMART',
  'PRO_Family_PND_Advance',
  'PRO_SFPP_Market_DEF',
  'PRO_SFPP_Market_FIT',
  'PRO_WPC_Product_Series_vivosmarthr',
  'PC1')

TrainedModel <- trainModel(methodtag, methodname, logging = F, repeats = reps, onlycolumns = selectedPreds)
TrainedModel
